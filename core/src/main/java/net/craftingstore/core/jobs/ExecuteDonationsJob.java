...
public class ExecuteDonationsJob {
    ...
    private void execute() throws CraftingStoreApiException {
        this.instance
                .getLogger()
                .debug(String.format("Executing ExecuteDonationsJob for %d donations.", this.donations.length));
        ...
        for (int i = 0; i < this.donations.length; i++) {
            ...
            this.instance
                    .getLogger()
                    .debug(String.format(
                            "Executing donation #%d with command id #%d",
                            donation.getPaymentId(), donation.getCommandId()));
            ...
            if (i < (this.donations.length - 1)) {
                if (!result) {
                    this.instance
                            .getLogger()
                            .debug("Not delaying command execution because last command did not successfully execute.");
                    continue;
                }
                ...
                try {
                    int waitingTime =
                            this.instance.getImplementation().getConfiguration().getTimeBetweenCommands();
                    this.instance
                            .getLogger()
                            .debug(String.format("Waiting %dms before executing next donation", waitingTime));
        ...
        int[] completedIds = donations.entrySet().stream()
                ...
                .mapToInt(entry -> entry.getKey().getPaymentId())
                .toArray();
        ...
        for (Map.Entry<Donation, Boolean> entry : donations.entrySet()) {
            ...
            if (entry.getValue()
                    && instance.getPendingDonations().containsKey(entry.getKey().getPaymentId())) {
                instance.getPendingDonations().remove(entry.getKey().getPaymentId());
            } else if (!entry.getValue()) {
                // Add the donation to the list of pending donations so it gets executed when the player gets online.
                instance.getPendingDonations().put(entry.getKey().getPaymentId(), entry.getKey());
            }
        }
