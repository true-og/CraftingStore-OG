...
public class DonationRunner {
    ...
    public void runDonations() {
        ...
        this.executor.submit(() -> {
            ...
            try {
                Donation[] donationQueue =
                        this.craftingStore.getApi().getDonationQueue().get();
                this.craftingStore
                        .getLogger()
                        .debug(String.format("Found %d donations in queue.", donationQueue.length));
                ...
                for (int i = 0; i < chunked.length; i++) {
                    ...
                    this.craftingStore
                            .getLogger()
                            .debug(String.format(
                                    "Creating ExecuteDonationsJob for chunk %d/%d", i + 1, chunked.length));
                ...
                if (craftingStore.getLogger().isDebugging()) {
                    ...
                    craftingStore
                            .getLogger()
                            .info("Failed to retrieve donation. The plugin will retry every 30 minutes.");
                }
            }
        });
    }
