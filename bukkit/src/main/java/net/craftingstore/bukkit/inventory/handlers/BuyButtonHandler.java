			...
			this.instance.getCraftingStore().getImplementation().runAsyncTask(() -> {
				try {
					...
					if (packageInformation.getPrice() != itemBuyablePackage.getPrice()) {
						p.sendMessage(CraftingStoreBukkit.getPrefix() + "There was a problem with the payment data. Please try again later.");
						closeInventory(p);
						return;
					}
					...
					EconomyResponse response = this.instance.getVaultHook().getEconomy().withdrawPlayer(p, packageInformation.getPrice());
					if (! response.transactionSuccess()) {
						p.sendMessage(CraftingStoreBukkit.getPrefix() + "There was a problem with the transaction. Please try again later");
						closeInventory(p);
						return;
					}
					...
					try {
						...
						if (! rollbackResponse.transactionSuccess()) {
							p.sendMessage(CraftingStoreBukkit.getPrefix() + "There was a problem with the transaction, and we were unable to give you back your money. Please contact an administrator");
							this.instance.getCraftingStore().getLogger().error(String.format(
						...
						p.sendMessage(CraftingStoreBukkit.getPrefix() + "A problem occurred while creating your payment. Your money has been given back. Please try again later");
						closeInventory(p);
					}
				} catch (Exception exception) {
					exception.printStackTrace();
