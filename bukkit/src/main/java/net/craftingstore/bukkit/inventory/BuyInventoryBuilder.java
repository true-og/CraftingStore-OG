...
public class BuyInventoryBuilder {
    ...
    public Inventory build(Player p, InventoryItemBuyablePackage item, CraftingStoreInventoryHolder holder)
            throws CraftingStoreApiException, ExecutionException, InterruptedException {
        ApiInventory buyMenu =
                this.instance.getCraftingStore().getApi().getGUI().get().getBuyMenu();
        CraftingStoreInventory craftingStoreInventory =
                new CraftingStoreInventory(buyMenu.getTitle(), buyMenu.getContent(), buyMenu.getSize());

        String ip = p.getAddress().getAddress().getHostAddress();
        ApiPackageInformation packageInformation = this.instance
                .getCraftingStore()
                .getApi()
                .getPackageInformation(p.getName(), p.getUniqueId(), ip, item.getPackageId())
                .get();
        ...
        placeholders.put("package_price_ingame", String.valueOf(packageInformation.getPrice()));
        return this.inventoryBuilder.buildInventory(
                craftingStoreInventory, new BuyMenuInventoryHolder(craftingStoreInventory, holder, item), placeholders);
    }
    ...
    public Inventory buildLoadInventory() {
        ...
        CraftingStoreInventory craftingStoreInventory = new CraftingStoreInventory("Loading...", content, 9);
        return this.inventoryBuilder.buildInventory(
                craftingStoreInventory, new CraftingStoreInventoryHolder(craftingStoreInventory, null));
    }
