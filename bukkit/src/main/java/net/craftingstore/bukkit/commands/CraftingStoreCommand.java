...
public class CraftingStoreCommand implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		if (!sender.hasPermission(instance.getCraftingStore().ADMIN_PERMISSION)) {
			TextUtil.craftingStoreMessage(sender, CraftingStoreBukkit.getPrefix() + "You don't have the required permission!");
			return false;
		}
			...
			instance.getServer().getScheduler().runTaskAsynchronously(instance, () -> {
				try {
					if (instance.getCraftingStore().reload().get()) {
						TextUtil.craftingStoreMessage(sender, CraftingStoreBukkit.getPrefix() + "The new API key has been set in the config, and the plugin has been reloaded.");
					} else {
						TextUtil.craftingStoreMessage(sender, CraftingStoreBukkit.getPrefix() + "The API key is invalid. The plugin will not work until you set a valid API key.");
					}
		...
		if ((args.length == 1 || args.length == 2) && args[0].equalsIgnoreCase("debug")) {
			...
			if (args.length == 1) {
				TextUtil.craftingStoreMessage(sender, String.format(
