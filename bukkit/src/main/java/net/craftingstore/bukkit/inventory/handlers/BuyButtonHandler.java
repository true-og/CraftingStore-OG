        ...
        if (action.equals(InventoryItemBuyButton.InventoryItemBuyButtonAction.IN_GAME)) {
            ...
            this.instance.getCraftingStore().getImplementation().runAsyncTask(() -> {
                try {
                    ...
                    ApiPackageInformation packageInformation = this.instance
                            .getCraftingStore()
                            .getApi()
                            .getPackageInformation(p.getName(), p.getUniqueId(), ip, itemBuyablePackage.getPackageId())
                            .get();
                    if (!packageInformation.isAllowedToBuy()) {
                        TextUtil.craftingStoreMessage(p, packageInformation.getMessage());
                        for (String message : itemBuyablePackage.getMessages()) {
                            TextUtil.craftingStoreMessage(p, message);
                        }
                        closeInventory(p);
                    ...
                    if (packageInformation.getPrice() != itemBuyablePackage.getPrice()) {
                        p.sendMessage(CraftingStoreBukkit.getPrefix()
                                + "There was a problem with the payment data. Please try again later.");
                        closeInventory(p);
                        return;
                    }
                    ...
                    EconomyResponse response =
                            this.instance.getVaultHook().getEconomy().withdrawPlayer(p, packageInformation.getPrice());
                    ...
                    try {
                        boolean success = this.instance
                                .getCraftingStore()
                                .getApi()
                                .createPayment(p.getName(), packageInformation.getPrice(), new int[] {
                                    itemBuyablePackage.getPackageId()
                                })
                                .get();
                        if (success) {
                            TextUtil.craftingStoreMessage(p, itemBuyablePackage.getSuccessMessage());
                            closeInventory(p);
                            return;
                        }
                        throw new RuntimeException(
                                "Failed to create a transaction at CraftingStore. Giving money back.");
                        ...
                        this.instance
                                ...
                                .error(String.format(
                                        ...
                                        p.getName(), itemBuyablePackage.getPackageId(), packageInformation.getPrice()));
                        EconomyResponse rollbackResponse = this.instance
                                .getVaultHook()
                                .getEconomy()
                                .depositPlayer(p, packageInformation.getPrice());
                        if (!rollbackResponse.transactionSuccess()) {
                            p.sendMessage(
                                    CraftingStoreBukkit.getPrefix()
                                            + "There was a problem with the transaction, and we were unable to give you back your money. Please contact an administrator");
                            this.instance
                                    .getCraftingStore()
                                    .getLogger()
                                    .error(String.format(
                                            "Unable to give the player's money back. Player: %s, cost: %s.",
                                            p.getName(), packageInformation.getPrice()));
                            closeInventory(p);
                            return;
                        }
                        ...
                        p.sendMessage(
                                CraftingStoreBukkit.getPrefix()
                                        + "A problem occurred while creating your payment. Your money has been given back. Please try again later");
                        closeInventory(p);
                    }
                } catch (Exception exception) {
                    exception.printStackTrace();
                    closeInventory(p);
                    p.sendMessage(CraftingStoreBukkit.getPrefix() + "A problem occurred. Please try again later.");
                }
            });
        }
    }
